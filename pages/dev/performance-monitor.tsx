...
const PerformanceMonitorPage: React.FC = () => {
  ...
  const generateMockData = () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      newMetrics.push({
        id: `metric-${i}`,
        timestamp, cpu, memory, fps, loadTime, networkRequests, networkTime
      });
      
      newChartData.push({
        name: timestamp.toLocaleTimeString(),
        cpu, memory: memory / 10, fps, loadTime: loadTime / 10
      });
    }
                  ...
                  {metrics.map((metric) => (
                    <div key={metric.id} className="p-4 hover:bg-muted/20">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2"><span className="text-sm font-medium">{metric.timestamp.toLocaleString()}</span></div>
                        <Badge variant={metric.cpu > 50 ? "destructive" : "outline"}>CPU: {metric.cpu}%</Badge>
                      </div>
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        <div className="flex justify-between"><span className="text-sm text-muted-foreground">Memory:</span><span className="text-sm font-medium">{metric.memory} MB</span></div>
                        <div className="flex justify-between"><span className="text-sm text-muted-foreground">FPS:</span><span className="text-sm font-medium">{metric.fps}</span></div>
                        <div className="flex justify-between"><span className="text-sm text-muted-foreground">Load Time:</span><span className="text-sm font-medium">{metric.loadTime} ms</span></div>
                        <div className="flex justify-between"><span className="text-sm text-muted-foreground">Network Requests:</span><span className="text-sm font-medium">{metric.networkRequests}</span></div>
                      </div>
                    </div>
                  ))}
                </div>
