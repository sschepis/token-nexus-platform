...
export default async function handler(
  ...
  if (username === bootstrapUsername && password === bootstrapPassword) {
    try {
      ...
      if (configCount === 0) {
        console.log('API bootstrap-login: No PlatformConfig found. Creating initial platform configuration...');
        
        // Create initial PlatformConfig using the ensureCoreInfrastructure cloud function
        try {
          await Parse.Cloud.run('ensureCoreInfrastructure', {}, { useMasterKey: true });
          console.log('API bootstrap-login: Initial PlatformConfig created successfully.');
        } catch (createError) {
          console.error('API bootstrap-login: Failed to create initial PlatformConfig:', createError);
          return res.status(500).json({
            success: false,
            message: 'Failed to initialize platform configuration.'
          });
        }
      }
      
      // Platform config now exists, proceed with bootstrap authentication
